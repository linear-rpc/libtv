...
void connection_cb(tv_stream_t* server, tv_stream_t* client, int status) {
  ...
  if (tv_ssl_get_verify_result(((tv_wss_t*)client)->ssl_handle)) {
    fprintf(stderr, "verify failed\n");
    tv_close((tv_handle_t*) client, close_cb);
    return;
  }
  cert = tv_ssl_get_peer_certificate(((tv_wss_t*)client)->ssl_handle);
  xname = X509_get_subject_name(cert);
  X509_NAME_get_text_by_NID(xname, NID_commonName, buf, sizeof(buf));
  fprintf(stderr, "Subject: CN=%s\n", buf);
  cipher = tv_ssl_get_current_cipher(((tv_wss_t*)client)->ssl_handle);
...
int main() {
  ...
  tv_ssl_library_init();
  ssl_ctx = SSL_CTX_new(TLSv1_1_server_method()); assert(ssl_ctx != NULL);
